...
class Ajax_controller_principal extends MY_Controller {
    ...
    function add_holiday() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $startDate = DateTime::createFromFormat('d-m-Y', $holidayDataArr['startDate']);
            $holidayDataArr['startDate'] = $startDate->format('Y-m-d');
            $endDate = DateTime::createFromFormat('d-m-Y', $holidayDataArr['endDate']);
            $holidayDataArr['endDate'] = $endDate->format('Y-m-d');
            //pre($holidayDataArr); die;
            $holidayId = $this->Sc_holiday_model->add($holidayDataArr);
            if ($holidayId != "") {
                echo json_encode(array('result' => 'good', 'msg' => 'holiday added successfully.'));die;
            }else{
                echo json_encode(array('result' => 'bad', 'msg' => 'Unknown Error.'));die; 
            }
        }
    }
